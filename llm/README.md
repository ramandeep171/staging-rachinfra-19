...
## Odoo 19 Enterprise Deployment Checklist

1. **Update sources** – sync the `19.0` branch on the server that runs your Odoo Enterprise instance and keep this repository inside the same filesystem as the core code.
2. **Install Python requirements** – inside the virtualenv used by Odoo run `pip install -r requirements.txt` so that shared dependencies (notably `pydantic 2.x`) are available before the registry loads these modules.
3. **Verify `addons_path`** – confirm that `odoo.conf` includes `custom_addons` (and this repository) before restarting the Odoo service.
4. **Initialize the stack** – install the modules in order with `./odoo-bin -c odoo.conf -i llm,llm_tool,llm_mcp --stop-after-init` or through the Apps UI.
5. **Grant security rights** – assign administrators to `Settings → Users & Companies → Users → Access Rights → LLM Manager` so they can configure providers and tools.
6. **Smoke-test the UI** – open the new **LLM** menu, fetch a provider, and tail `odoo19e.log` to confirm there are no access or registry errors.
...
1. Follow the deployment checklist above to make sure dependencies are installed and the addons path is updated.
2. Install `LLM Integration Base` from the Apps screen or run `./odoo-bin -c odoo.conf -i llm --stop-after-init`.
3. (Optional but recommended) Install `llm_tool` and `llm_mcp` so that tool execution and MCP servers are available immediately.
...
1. Open *Settings → Technical → Email → Subtypes* and confirm that the `llm.mt_*` subtypes exist.
2. Post a message on any chatter thread and check that the form view shows the computed `LLM Role` field.
3. Run a quick provider test: create an OpenAI provider, click **Fetch Models**, and ensure that models are created without errors in the log.
4. If `llm_tool` is installed, go to *LLM → Configuration → Tools* and use **Reset Input Schema** on a sample tool to verify that schemas are generated.
...
**Version 19.0.1.0.0:**
- Ported the module to the Odoo 19 Enterprise codebase and aligned dependencies.
- Added deployment checklist plus validation steps for llm_tool and llm_mcp integrations.
