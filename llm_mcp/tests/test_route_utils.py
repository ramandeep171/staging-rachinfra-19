...
def test_route_paths_from_env(monkeypatch):
    ...
    paths = route_utils.mcp_route_paths("/mcp/tools")
    assert paths[:3] == [
        "/mcp/tools",
        "/odoo/mcp/tools",
        "/api/v1/mcp/tools",
    ]
    assert paths[-1] == "/<path:mcp_proxy_prefix>/mcp/tools"


def test_route_paths_from_config(monkeypatch):
    _clear_env(monkeypatch)
    monkeypatch.setattr(route_utils, "odoo_config", {"llm_mcp_route_prefix": "/core"})
    paths = route_utils.mcp_route_paths("/mcp/execute")
    assert paths[:2] == [
        "/mcp/execute",
        "/core/mcp/execute",
    ]
    assert paths[-1] == "/<path:mcp_proxy_prefix>/mcp/execute"
