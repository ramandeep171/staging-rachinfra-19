...
class MrpProduction(models.Model):
    ...
    def _check_x_subcontractor_id(self):
        for production in self:
            if production.x_subcontractor_id and production.product_id:
                allowed = production.product_id.allowed_subcontractor_ids
                if production.x_subcontractor_id not in allowed:
                    raise ValidationError(
                        _(
                            "The subcontractor '%(partner)s' is not allowed for the Brand value of '%(product)s'."
                        )
                        % {
                            "partner": production.x_subcontractor_id.display_name,
